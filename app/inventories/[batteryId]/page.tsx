import { notFound } from 'next/navigation';
import { and, eq, inArray } from 'drizzle-orm';
import Link from 'next/link';
import { db, questionAnswers } from '@/db';
import { getBattery } from '@/lib/inventory/registry';
import type { AnswerValue } from '@/lib/inventory/types';
import { BatteryForm } from './battery-form';

function todayISO(): string {
  const d = new Date();
  const yyyy = d.getFullYear();
  const mm = String(d.getMonth() + 1).padStart(2, '0');
  const dd = String(d.getDate()).padStart(2, '0');
  return `${yyyy}-${mm}-${dd}`;
}

export default async function BatteryPage({
  params,
}: {
  params: Promise<{ batteryId: string }>;
}) {
  const { batteryId } = await params;
  const battery = getBattery(batteryId);
  if (!battery) notFound();

  const forDate = todayISO();
  const rows = await db
    .select({ questionId: questionAnswers.questionId, value: questionAnswers.value })
    .from(questionAnswers)
    .where(
      and(
        eq(questionAnswers.forDate, forDate),
        inArray(
          questionAnswers.questionId,
          battery.questions.map((q) => q.id),
        ),
      ),
    );

  const initial: Record<string, AnswerValue> = {};
  for (const r of rows) initial[r.questionId] = r.value;

  return (
    <div className="flex flex-col gap-6 p-10">
      <div className="flex flex-col gap-2">
        <Link
          href="/inventories"
          className="text-xs text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-50"
        >
          ← Inventories
        </Link>
        <h1 className="text-2xl font-semibold tracking-tight">{battery.name}</h1>
        {battery.description && (
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            {battery.description}
          </p>
        )}
        <p className="text-xs text-zinc-400 dark:text-zinc-500">For {forDate}</p>
      </div>

      <BatteryForm batteryId={battery.id} initial={initial} />
    </div>
  );
}
